...
<ResponseField name="actions" type="object">
  ...
  `moveNode`, `selectNode`, `setMode`, `updateVariables`.
</ResponseField>

<ResponseField name="updateVariables" type="(variables: Record&lt;string, { fallback: string }&gt;) => void">
  Updates the document's variable definitions. Call this to add, modify, or remove variables programmatically.
</ResponseField>
...
Get the current document's variable definitions.

```tsx
import { useVariables } from "@open-email/editor";

const variables = useVariables();
// Returns: Record<string, { fallback: string }>
```

This hook returns the `variables` object from the current document, which contains all variable definitions with their fallback values.
